    ...
    >
      ...
      >
          ...
          <Grid item className={classes.nameage}> 
            <Box display="flex" alignItems="baseline">
              <Box mr={2} maxWidth="300px" textOverflow="clip">
                <Typography variant="h5" align='left'> {props.firstname} </Typography>
              </Box>
        ...
        <Grid item sm = {7} xs={12} className={classes.interestText}>          
          <Typography variant='h5' style={{color: "#ff7a45"}}> Lifestyle </Typography>
        </Grid>
        <Grid item sm={5} xs={6} className={mobile ? classes.image : classes.interestImage}>
          <img className={classes.imgRight} src="https://www.uclg-planning.org/sites/default/files/styles/featured_home_left/public/no-user-image-square.jpg"/>
        </Grid>
        <Grid item xs={12} style={{ padding: "24px 24px 24px 24px"}}>
          {interests.map((interest, index) => (
            index < 10 && 
            <React.Fragment key={index}>
              <Chip label={interest} variant='outlined' style={{ margin: "4px", borderColor: "#ff7a45"}}/>
            </React.Fragment>          
          ))}
        </Grid>
